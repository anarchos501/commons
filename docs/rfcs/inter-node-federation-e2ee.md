...
| F3.5 | Continuity: establishment + advance directive, structural delta replication, Tier-1 read-only, Tier-2 lease + coordination annex, quiet-boot + catch-up, key escrow | Partially implemented — establishment + consent matrix, key escrow, structural delta replication, and Tier 1 (stateless lease resolver with mirror self-demotion, resolver/guard/page gates, heartbeat, challenge with relay + witness proof-of-life, replica page) are built; Tier 2 (activation, takeover log, quiet-boot + catch-up) is specified, not yet built |
