...
| F3 | Visibility grants (implemented, `ae556de`); nullable-membership migration isolated (`4e809e4`); cross-node coalitions (implemented, `e256e2d` — hub relay proven over three databases with the hub-ness precondition asserted, membership XOR as a DB CHECK constraint); node tags implemented (shared `NodeTag` origin badge; home-side coalition page renders remote members; routed-write copy on the remote post form); dashboard aggregation deliberately deferred | Partially implemented |
...
| F3.5 | Continuity: establishment + advance directive, structural delta replication, Tier-1 read-only, Tier-2 lease + coordination annex, quiet-boot + catch-up, key escrow | Implemented for groups (Phases 0–4): establishment + consent matrix, key escrow with the stranded-login primitive, structural delta replication, Tier-1 lease (stateless resolver, mirror self-demotion, challenge/relay/witness proof-of-life), Tier-2 log-only takeover (two-verb annex, expedite + graceful-handoff accelerators, contested-activation convergence), quiet-boot over the signed continuity-status pull with chunked catch-up and cede (`ea77370`, `37e196d`, `4677620`, `8b12aeb`). Phase 5 landed: project entrance (project-native petition), coalition designation as a full coalition decision through every member group's own governance — remote members via the cross-node proposal machinery, peer choice part of the consent — with consent-withdrawal revocation (any one member group, local or remote, alone) and the coalition write-authority gate on all four write paths. The browser-side escrow-unwrap UI matures at F4 |
