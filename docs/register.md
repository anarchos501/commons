...
**Consequences:** The designation carries the failover window W and an **advance directive** (`reconstitute` | `none`) — the entity's pre-consent to post-disaster re-homing, collected at the only moment its own machinery can legitimately decide it (afterwards, the machinery *was* the home). Tier-2 is a **coordination annex** (ratified narrowing): a signed append-only log with a registered two-verb vocabulary; petitions freeze on both sides; "actable for discussion," stated on the banner. A backed-up entity's replica members are shadow-relative to the backup node (zero weight, D-5). Tier 3 — two-live-writer partition merge, and the old member-quorum disaster activation — is post-beta, named here, not built. Dependencies named: the D-8 key escrow is what the directive's execution, Tier-2 verified actors, and reconstitution stand on. Operational note: every deploy/restart causes a seconds-long read-only blip for backed-up entities until quiet-boot verification round-trips — fail-safe, intended. Project and coalition designation entrances LANDED (Phase 5) — the owed rider is discharged. **Coalition designation is a coalition decision through every member's own governance (no tiering by data locality):** membership in a coalition is not tiered by where a group's data happens to live — every member group, local or remote, consents to a backup designation through its own node's petitions (the cross-node proposal machinery), and the PEER CHOICE is part of that consent, collected at the only pre-disaster moment reconstitution can legitimately draw on. The replica holds the coalition's home-side state only (skeleton, relay thread, proposal history), never member groups' own data — each group's backups are its own node's decisions. **Revocation is consent-withdrawal:** the designation stands on unanimous consent, so any ONE member group's withdrawal — through its own petition, with the same unilateral door local and remote — revokes it for everyone (stop cheaper than start, F-2). **Calm-weather property (stated at the designation form and in the guide):** full-member consent means a coalition can only arrange its disaster protection while ALL its member nodes are healthy — a proposal with an unreachable member times out, because consent cannot be presumed from silence. Designate early, in calm weather.
