...
import { upsertGovernanceSignal, clearGovernanceSignal, computeGroupTemperature, SIGNAL_CHANGE_COOLDOWN_HOURS } from "../lib/governance-temperature";
...
test("upsertGovernanceSignal creates signal record", async () => {
  ...
  try {
    ...
    const record = await prisma.memberGovernanceSignal.findUnique({ where: { membershipId_category: { membershipId: memberships[0].id, category: "membership" } } });
    assert.ok(record);
...
test("resolver returns temperature-interpolated threshold after signal set", async () => {
  ...
  try {
    // All members signal +1 → temperature = 1.0 → threshold = permissive = 0.40
    await upsertGovernanceSignal(prisma, { membershipId: memberships[0].id, groupId: group.id, category: "membership", signal: 1 });
    ...
    assert.ok(Math.abs(params.threshold - 0.40) < 0.001, `Expected 0.40, got ${params.threshold}`);
  } finally {
